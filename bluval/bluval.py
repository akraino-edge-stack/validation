...
def invoke_docker(bluprint, layer):
    ...
    cmd = ("sudo docker run"
            ...
            " -v $VALIDATION_HOME/tests/variables.yaml:/opt/akraino/validation/tests/variables.yaml"
            " -v $AKRAINO_HOME/results:/opt/akraino/results"
            " akraino/validation:{0}-latest"
            " bin/sh -c"
            " 'cd /opt/akraino/validation && python bluval/bluval.py -l {0} rec'").format(layer)
    args = [cmd]
    try:
        print('Invoking {}'.format(args))
...
@click.option('--deligate', '-d', is_flag=True)
def main(blueprint, layer, deligate):
