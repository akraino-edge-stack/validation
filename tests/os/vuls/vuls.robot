...
Run Vuls test
    ...
    Run Keyword IF  '${SSH_KEYFILE}' == 'None'  Get ssh_keyfile
...
Get ssh_keyfile
    ${ssh_output} =  SSHLibrary.Execute Command  ls -la /root/.ssh
    ${status} =  Evaluate  "id_rsa" in """${ssh_output}"""
    Run Keyword If  '${status}' == 'False'  SSHLibrary.Execute Command  ssh-keygen -t rsa -b 4096 -f /root/.ssh/id_ras -N validation

    SSHLibrary.Get File  /root/.ssh/id_rsa  /root/.ssh/
