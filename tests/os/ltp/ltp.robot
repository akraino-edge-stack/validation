...
Run LTP syscalls madvise only
    [Documentation]         Wait ~1m for madvise01-10 to complete
    ${log} =  Set Variable  ${LOG_PATH}${/}${SUITE_NAME.replace(' ','_')}_syscalls_madvise.log
...
Run LTP syscalls only
    [Documentation]         Wait ~45m for syscalls to complete
    ${log} =  Set Variable  ${LOG_PATH}${/}${SUITE_NAME.replace(' ','_')}_syscalls.log
...
Run LTP all tests
    ...
    ${log} =  Set Variable  ${LOG_PATH}${/}${SUITE_NAME.replace(' ','_')}_whole.log
