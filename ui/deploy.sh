...
CONTAINER_NAME="validation-ui-dev"
REGISTRY=akraino
NAME=validation
TAG_PRE=dev-ui
TAG_VER=latest
...
docker run --name $CONTAINER_NAME --network="host" -it --rm -e db_connection_url="$db_connection_url" -e maria_db_user_pwd="$mariadb_user_pwd" -e jenkins_url="$jenkins_url" -e jenkins_user_name="$jenkins_user_name" -e jenkins_user_pwd="$jenkins_user_pwd" -e jenkins_job_name="$jenkins_job_name" $IMAGE
sleep 10
