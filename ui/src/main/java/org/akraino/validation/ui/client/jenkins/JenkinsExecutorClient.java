...
public final class JenkinsExecutorClient {
    ...
    public QueueJobItem getQueueJobItem(URL queueJobItemUrl) {
        synchronized (LOCK) {
            ...
            try {
                ...
                response = builder.accept("application/json").type("application/json").get(ClientResponse.class);
            } catch (ClientHandlerException ex) {
                logger.error("Connection error. ", ex);
                return null;
            }
            if (response.getStatus() != 200) {
                logger.error("Get on Jenkins failed. HTTP error code : " + response.getStatus() + " and message: "
     ...
     * @param jobName
     * @param parameters
     * @return The URL of the corresponding queue job item
     */
