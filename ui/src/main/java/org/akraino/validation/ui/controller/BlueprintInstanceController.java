...
public class BlueprintInstanceController extends RestrictedBaseController {
    ...
    private EELFLoggerDelegate LOGGER = EELFLoggerDelegate.getLogger(BlueprintInstanceController.class);

    @Autowired
    BlueprintInstanceService blueprintInstanceService;
    ...
    public ResponseEntity<List<BlueprintInstance>> getBlueprintInstances() {
        try {
            return new ResponseEntity<>(blueprintInstanceService.getBlueprintInstances(), HttpStatus.OK);
        } catch (Exception e) {
