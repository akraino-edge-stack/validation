...
public class SubmissionController {
    ...
    public ResponseEntity<List<Submission>> getSubmissions() {
        try {
            return new ResponseEntity<>(submissionService.getSubmissions(), HttpStatus.OK);
        } catch (Exception e) {
            logger.error(e);
        }
    ...
    public ResponseEntity<Submission> postSubmission(@RequestBody Submission newSubmission) {
        try {
            return new ResponseEntity<>(submissionService.saveSubmission(newSubmission), HttpStatus.CREATED);
        } catch (Exception e) {
            logger.error(e);
        }
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(null);
    ...
    public ResponseEntity<Boolean> deleteSubmission(@RequestBody Submission submission) {
        try {
            submissionService.deleteSubmission(submission.getSubmissionId());
