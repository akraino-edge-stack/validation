...
public class SubmissionService {
    ...
    public Submission saveSubmission(Submission submission) {
        ...
        CompletableFuture<Submission> completableFuture =
                CompletableFuture.supplyAsync(new PrioritySupplier<>(1, task::execute), UiConfig.executorService);
    ...
    private class JenkinsTriggerSubmissionJob {
        ...
        public Submission execute() {
            ...
            parameter.setValue(submission.getBlueprintInstance().getBlueprint().getBlueprintName().toLowerCase());
            listOfParameters.add(parameter);
            parameter = new Parameter();
            parameter.setName("LAYER");
            parameter.setValue(submission.getBlueprintInstance().getLayer().name().toLowerCase());
            listOfParameters.add(parameter);
            parameter = new Parameter();
            ...
            try (final DatagramSocket socket = new DatagramSocket()) {
                ...
                LOGGER.error(EELFLoggerDelegate.errorLogger, "Error when triggering Jenkins job", e);
                return null;
            }
        }
    }
