...
public class ResultsController extends RestrictedBaseController {
    ...
    private EELFLoggerDelegate LOGGER = EELFLoggerDelegate.getLogger(ResultsController.class);
    ...
    public ResponseEntity<List<RobotTestResult>> getByBlueprintId(@PathVariable("id") String submissionId) {
        try {
            return new ResponseEntity<>(service.getRobotTestResults(submissionId), HttpStatus.OK);
        } catch (Exception e) {
            LOGGER.error(EELFLoggerDelegate.errorLogger, "Error when retrieving results", e);
        }
