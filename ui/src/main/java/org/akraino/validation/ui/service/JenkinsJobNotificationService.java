...
public class JenkinsJobNotificationService {
    ...
    public void handle(JnksJobNotify jnksJobNotify) {
        ...
        submission.setNexusResultUrl(
                UiConfig.nexusUrl + "/" + submission.getBlueprintInstance().getTimeslot().getLab().name().toLowerCase()
                        + "-blu-val" + "/job/validation/" + String.valueOf(jnksJobNotify.getbuildNumber() + "/results/"
