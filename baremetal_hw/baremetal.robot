...
Resource          baremetal.resource
Library           OperatingSystem
Library           BuiltIn
Library           Process

*** Test Cases ***
Connectivity to master host
    ${output}=        Run    ping ${HOST_MR} -c 5
    Append To File    ${LOG}  ${output}${\n}
    Should Contain    ${output}    5 packets transmitted, 5 received

Connectivity to worker_1 host
    ${output}=        Run    ping ${HOST_WR1} -c 5
    Append To File    ${LOG}  ${output}${\n}
    Should Contain    ${output}    5 packets transmitted, 5 received

Connectivity to worker_2 host
    ${output}=        Run    ping ${HOST_WR2} -c 5
    Append To File    ${LOG}  ${output}${\n}
    Should Contain    ${output}    5 packets transmitted, 5 received
